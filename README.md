...
requirements.txt
.env.example           — пример переменной окружения для токена бота
...
```bash
cp .env.example .env
# откройте .env и впишите реальный токен
...
- `url` — адрес страницы, которая проверяется. Если товары на разных
...
- `history_file` / `log_file` — пути к файлам истории и лога (необязательные,
  по умолчанию `history.csv` и `price_monitor.log` рядом со скриптом).
...
(`#product-1`, `#product-2`), которую можно опубликовать на GitHub Pages:
